function output=histStat(bincenters,counts,nMoments)
...
for n=1:nMoments
    ...
    output{n}=EXn(1,:).^(n); % k=0 term in binomial theorem
    if n>1 %not centering the mean
        for k=1:n
            output{n}=output{n}+nchoosek(n,k)*EXn(k,:).*EXn(1,:).^(n-k); %nchooseek = binomial coefficient
        end
    end
end
