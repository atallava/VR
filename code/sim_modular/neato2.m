classdef neato2 < handle
	% Abhijeet's sim built on Mike's
	...
	methods
		function r = neato2(name,pose)
			...
			if(strcmp(name,'sim') || strcmp(name,'manual_sim') )
				...
				r.sim_robot = constructSimRobotFromComponents(components);
				% simRobot
% 				r.sim_robot =  simRobot(r.enc_delay, initialPose, 0, 0, false);
				...
				if(~strcmp(name,'manual_sim'))
					...
					r.update_timer.ExecutionMode = 'fixedSpacing';
				...
				try
					...
					error(['Cant open socket, first try to "clear all", then try again'...
						...
						'connected to the network, or the robot needs to be restarted'])
				end
		...
		function shutdown(r)
			% Close connection and shutdown robot.
			% Write to usage log.
			fname = sprintf('%s/%s',r.logDirName,neato2.logFileName);
			if ~exist(fname,'file')
				fid = fopen(fname,'w');
			else
				fid = fopen(fname,'a');
		...
		function r = close(r)
			% Close connection but keep robot alive.
			if(~r.sim)
				r.ws.delete;
			end
		end
		...
		function setMap(r,map)
			r.map = map;
			r.sim_robot.setMap(map);
			r.has_map = true;
		end
	...
	methods (Hidden = true, Access = 'private')
		function constructSimRobot(r)
			fname = 'sim_config.xml';
		...
		function simUpdate(r,caller,event)
			...
			% compute distance traveled since last update step
			...
			if(r.enc_cnt == r.enc_prescaler)
				...
				r.encoders.data = r.sim_robot.getEncoderData();
			...
			if(r.laser_cnt == r.laser_prescaler)
				if r.sim_laser_on
					r.laser.data.ranges = r.sim_robot.getLaserReadings();
				end
